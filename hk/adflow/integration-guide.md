...
| **需要了解 Prebid.js** | 否 | 是 |
| **支持廣告格式** | Banner / Pop / Push / In-page Push | Banner / Video / Native / Multi-Format |
...
adflow.js 是一个自包含的 JavaScript SDK，将 Prebid.js S2S 竞价的所有逻辑封装在一个脚本中。客户只需引入一个 `<script>` 标签并在廣告位放置 `<iframe>` 标签即可完成接入，无需手动加载 Prebid.js 或編寫竞价代码。支持 **Banner**（橫幅）、**Pop**（彈窗）、**Push**（推送通知）、**In-page Push**（頁內推送）四種廣告類型，通過廣告位上的 `data-ad-type` 屬性切換。

[Banner 調試器](/hk/adflow/banner-debugger) · [Pop 調試器](/hk/adflow/pop-debugger) · [Push 調試器](/hk/adflow/push-debugger) · [In-page Push 調試器](/hk/adflow/inpage-push-debugger)
...
adflow.js 支持四種廣告類型，腳本引入方式完全相同，區別僅在於廣告位 `<iframe>` 上的 `data-ad-type` 屬性。
...
| `data-ad-type` | 可选 | `banner` | 廣告類型：`banner`（默認）、`pop`（彈窗）、`push`（推送通知）、`inpage-push`（頁內推送） |
| `width` | 可选 | `300` / `492` | 廣告位宽度(像素)。`banner` 為創意尺寸（默認 300）；`inpage-push` 為請求的圖片寬度（默認 492） |
| `height` | 可选 | `250` / `328` | 廣告位高度(像素)。`banner` 為創意尺寸（默認 250）；`inpage-push` 為請求的圖片高度（默認 328） |
