---
title: Native 調試器
description: 在瀏覽器中測試 adflow.js Native（原生資訊流）廣告。配置 URL、賬號 ID、廣告位以驗證 S2S 競價。
---

# Native 調試器

測試 adflow.js 對 **Native（原生資訊流）**廣告類型的支持：競價請求與 Push 相同（圖片 + 標題 + 正文的原生格式），但廣告**不彈窗** —— 中標後 adflow.js 會在**廣告位所在位置**直接插入一張廣告卡片，與頁面內容自然融合。
...
<div class="nat-dbg">
  <div class="controls">
    ...
    <button id="nat-runBtn">運行測試</button>
  </div>
</div>
</ClientOnly>

::: tip 無需授權
Native 廣告不使用瀏覽器通知，無需 HTTPS 或通知權限。中標後卡片直接內嵌渲染在下方廣告位中 —— 圖片在上、標題正文在下，無關閉按鈕。
:::

## 代碼預覽

根據上方配置自動生成的接入代碼。原生廣告位與普通廣告位的唯一區別就是 `data-ad-type="native"`；`width` / `height` 可選，定義請求的圖片尺寸（默認 492×328）：
...
<div class="nat-dbg">
  <div class="grid">
    <div class="card">
      <h3>執行狀態</h3>
      <div>
        <div class="test-row"><span class="test-name">腳本加載</span><span class="status pending" id="nat-status-script">等待測試</span></div>
        <div class="test-row"><span class="test-name">配置讀取</span><span class="status pending" id="nat-status-config">等待測試</span></div>
        <div class="test-row"><span class="test-name">廣告位發現</span><span class="status pending" id="nat-status-slots">等待測試</span></div>
        <div class="test-row"><span class="test-name">競價</span><span class="status pending" id="nat-status-auction">等待測試</span></div>
        <div class="test-row"><span class="test-name">中標</span><span class="status pending" id="nat-status-render">等待測試</span></div>
        <div class="test-row"><span class="test-name">內嵌卡片</span><span class="status pending" id="nat-status-notify">等待測試</span></div>
      </div>
    </div>
    <div class="card">
      <h3>運行時配置</h3>
...
<div class="nat-dbg">
  <div class="card" style="margin-bottom: 24px;">
    <h3>原生廣告位（卡片直接在下方內嵌渲染）</h3>
    <p style="font-size:14px;color:var(--vp-c-text-2);margin:0;">
      原生廣告不彈窗。中標後 adflow.js 會在<strong>廣告位所在位置</strong>直接插入一張
      廣告卡片（圖片 + 標題 + 正文，無關閉按鈕），點擊卡片打開落地頁。
      投放結果見上方「執行狀態」「內嵌卡片」「解析結果」。
    </p>
    <div class="ad-slot-label" style="margin-top:10px;">廣告位：data-ad-type="native" · <span id="nat-label-pid">test-native-1</span></div>
...
onMounted(async () => {
  ...
  function updateCodePreview() {
    ...
    html += '\n';
    html += '<span class="hl-comment">&lt;!-- 原生廣告位（width/height 可選，默認 492×328） --&gt;</span>\n';
  ...
  function pollForCompletion() {
    ...
    var timer = setInterval(function() {
      ...
      // Native 不在 iframe 內渲染任何內容；中標即為成功（adflow 隨後插入內嵌卡片）。
      if (nativeHasWinner()) {
        clearInterval(timer);
        finishTest(true);
        return;
  ...
  function finishTest(won) {
    if (won) {
      ...
      log('========== 測試完成: 已中標，原生卡片由 adflow.js 內嵌渲染 ==========', 'success');
    } else {
      setStatus('nat-status-render', 'warn', '無出價');
      setStatus('nat-status-notify', 'pending', '無廣告');
      log('========== 測試完成: 無原生廣告出價返回（測試環境屬正常現象） ==========', 'warn');
    }
    enableButtons();
  }
