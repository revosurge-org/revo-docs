...
| `fishing` | `fish`（捕魚）· `arcade`（街機） |
| `table_games` | `baccarat`（百家樂）· `blackjack`（二十一點）· `roulette`（輪盤）· `dice`（骰寶）· `card`（紙牌）· `table`（桌檯） |
| `poker` | `poker`（德州撲克） |
| `lottery` | `lottery`（彩票）· `bingo`（賓果）· `instant`（即開彩／刮刮樂） |
| `esports` | `esport`（電競） |
...
| `context.bonus_type` | String | **必填** | `welcome` · `reload` · `cashback` · `freespin` · `promo_code` · `tournament` · `voucher`（優惠券發放）· `promcash`（優惠金發放）· `other` 其中之一。 |
