...
iGaming 事件（`igaming` 預設集）涵蓋遊戲垂直領域：擴充的用戶生命週期、KYC、資金、遊戲及獎金生命週期。
...
| `context.rejection_reason` | String | **必填** | `document_invalid` · `face_mismatch` · `sanctions_hit` · `underage` · `duplicate` · `other` 其中之一。 |
