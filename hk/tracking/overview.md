...
::: details 可以在不產生真實數據的前提下驗證嗎？
可以。在 [S2S](/hk/tracking/s2s/v3/server-events-api#dry-run) 或 [Partner Postback](/hk/tracking/postback/partner-postback#dry-run) 端點上加 `?dryrun=1`，我們會完全按生產路徑解析、驗證並豐富你的請求，把將要記錄的內容原樣回顯 —— 但不落庫。
:::
