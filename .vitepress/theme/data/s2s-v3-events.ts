...
const base = (name: string, cuid: Req = 'required', ip: Req = 'required'): FieldDef[] => [
  ...
  f('anonymous_id', 'identity', 'String', 'optional', 'anon_7f3a',
    tri('Pre-registration anonymous ID; used for app-lifecycle stitching before client_user_id exists.', '注册前的匿名 ID；在 client_user_id 出现前用于 App 生命周期身份缝合。', '註冊前的匿名 ID；在 client_user_id 出現前用於 App 生命週期身份縫合。')),
...
export const s2sV3Events: EventDef[] = [
  ...
  def('register', 'standard', 'user_lifecycle', [
    ...
    f('phone_country_code', 'context', 'String', 'suggested', '+55',
      tri('E.164 country calling code (e.g. +55).', 'E.164 国家区号（如 +55）。', 'E.164 國家區號（如 +55）。')),
    f('consent_timestamp', 'context', 'Number', 'suggested', TS_SAMPLE,
      tri('Last consent change, UTC ms.', '最近一次授权变更，UTC 毫秒。', '最近一次授權變更，UTC 毫秒。')),
    f('attribution.utm_source', 'context', 'String', 'suggested', 'google',
      tri('UTM source.', 'UTM 来源。', 'UTM 來源。')),
    f('attribution.utm_medium', 'context', 'String', 'suggested', 'cpc',
      tri('UTM medium.', 'UTM 媒介。', 'UTM 媒介。')),
    f('attribution.utm_campaign', 'context', 'String', 'suggested', 'summer_promo',
      tri('UTM campaign.', 'UTM 活动。', 'UTM 活動。')),
    f('attribution.utm_content', 'context', 'String', 'suggested', 'banner_a',
      tri('UTM content.', 'UTM 内容。', 'UTM 內容。')),
    f('attribution.utm_term', 'context', 'String', 'suggested', 'slots',
      tri('UTM term.', 'UTM 关键词。', 'UTM 關鍵詞。')),
  ]),
  def('referral_register', 'standard', 'user_lifecycle', [
    f('referral_client_user_id', 'context', 'String', 'suggested', 'user_referrer_042',
      tri("The referrer's client_user_id (the user who referred this registration).", '推荐人的 client_user_id（推荐本次注册的用户）。', '推薦人的 client_user_id（推薦本次註冊的用戶）。')),
  ]),
  ...
  def('phone_verified', 'standard', 'verification_consent', [
    ...
    f('phone_country_code', 'context', 'String', 'suggested', '+91',
      tri('E.164 country calling code (e.g. +91).', 'E.164 国家区号（如 +91）。', 'E.164 國家區號（如 +91）。')),
    f('verification_method', 'context', 'String', 'suggested', 'sms_otp',
      tri('How the phone was verified. Prefer voice_call / whatsapp_otp; call / whatsapp are deprecated aliases.', '手机验证方式。建议用 voice_call / whatsapp_otp；call / whatsapp 为已弃用的旧值。', '電話驗證方式。建議用 voice_call / whatsapp_otp；call / whatsapp 為已棄用的舊值。'), { enumValues: ['sms_otp', 'voice_call', 'whatsapp_otp', 'call', 'whatsapp'] }),
  ]),
  ...
  def('app_install', 'standard', 'app_lifecycle', [
    ...
    f('store', 'context', 'String', 'suggested', 'play_store',
      tri('Install store: app_store / play_store / apk_direct.', '安装商店：app_store / play_store / apk_direct。', '安裝商店：app_store / play_store / apk_direct。')),
    f('campaign_id', 'context', 'String', 'suggested', 'cmp_123', tri('Campaign ID (from MMP).', '广告活动 ID（来自 MMP）。', '廣告活動 ID（來自 MMP）。')),
    f('is_reattribution', 'context', 'Boolean', 'suggested', false, tri('true if reinstall / reattribution.', '重装/重新归因为 true。', '重裝/重新歸因為 true。')),
  ...
  def('app_open', 'standard', 'app_lifecycle', [
    ...
    f('is_first_open', 'context', 'Boolean', 'suggested', true, tri('true for first open after install.', '安装后首次打开为 true。', '安裝後首次打開為 true。')),
  ...
  def('app_uninstall', 'standard', 'app_lifecycle', [
    ...
    f('days_since_install', 'context', 'Number', 'suggested', 30, tri('Days from install to uninstall.', '从安装到卸载的天数。', '從安裝到卸載的天數。')),
  ...
  def('push_permission_updated', 'standard', 'app_lifecycle', [
    ...
    f('push_enabled', 'context', 'Boolean', 'suggested', true, tri('true if push permission granted.', '已授予推送权限为 true。', '已授予推送權限為 true。')),
    f('update_source', 'context', 'String', 'suggested', 'system_prompt', tri('What triggered the change: system_prompt / settings / onboarding.', '变更触发来源：system_prompt / settings / onboarding。', '變更觸發來源：system_prompt / settings / onboarding。')),
  ]),
  ...
  def('session_ended', 'igaming', 'user_lifecycle', [
    ...
    f('total_wagered', 'context', 'Number', 'suggested', 250.0, tri('Total wagered in session.', '会话内总投注额。', '工作階段內總投注額。')),
    f('total_won', 'context', 'Number', 'suggested', 180.0, tri('Total won in session.', '会话内总赢额。', '工作階段內總贏額。')),
    f('bet_count', 'context', 'Number', 'suggested', 42, tri('Number of bets in session.', '会话内投注次数。', '工作階段內投注次數。')),
    f('currency', 'context', 'String', 'suggested', 'USD', tri('Required if wagered / won is sent.', '若上报 wagered / won 则必填。', '若上報 wagered / won 則必填。')),
    f('is_crypto', 'context', 'Boolean', 'suggested', false, D_CRYPTO),
    f('end_reason', 'context', 'String', 'suggested', 'user_logout', tri('Session end reason: user_logout / timeout / forced_logout / unknown.', '会话结束原因：user_logout / timeout / forced_logout / unknown。', '工作階段結束原因：user_logout / timeout / forced_logout / unknown。')),
  ]),
  def('vip_tier_changed', 'igaming', 'user_lifecycle', [
    ...
    f('previous_tier', 'context', 'String', 'suggested', 'silver', tri('Tier before this change.', '本次变更前的等级。', '本次變更前的等級。')),
  ...
  def('kyc_rejected', 'igaming', 'verification_consent', [
    ...
    f('jurisdiction', 'context', 'String', 'required', 'BR', tri('ISO 3166-1 country code.', 'ISO 3166-1 国家代码。', 'ISO 3166-1 國家代碼。')),
    f('rejection_reason', 'context', 'String', 'required', 'document_invalid', tri('Why KYC was rejected.', 'KYC 被拒原因。', 'KYC 被拒原因。'), { enumValues: ['document_invalid', 'face_mismatch', 'sanctions_hit', 'underage', 'duplicate', 'other', 'document_unreadable', 'document_expired', 'name_mismatch', 'age_restriction', 'suspected_fraud'] }),
    f('verification_method', 'context', 'String', 'suggested', 'id_document', tri('Verification method attempted.', '尝试的验证方式。', '嘗試的驗證方式。')),
    f('time_to_reject_minutes', 'context', 'Number', 'suggested', 45, tri('Minutes from register to this event.', '从 register 到本事件的分钟数。', '從 register 到本事件的分鐘數。')),
  ]),
  ...
  def('account_unblocked', 'igaming', 'verification_consent', [
    f('unblock_reason', 'context', 'String', 'required', 'review_passed',
      tri('Why the account was unblocked. Prefer cooldown_ended; temporary_expired is a deprecated alias.', '账户被解封的原因。建议用 cooldown_ended；temporary_expired 为已弃用的旧值。', '帳戶被解封的原因。建議用 cooldown_ended；temporary_expired 為已棄用的舊值。'), { enumValues: ['review_passed', 'appeal_approved', 'cooldown_ended', 'manual_admin', 'other', 'temporary_expired'] }),
  ...
  def('bonus_offered', 'igaming', 'bonus_lifecycle', [
    ...
    f('bonus_type', 'context', 'String', 'required', 'welcome', tri('Bonus type.', '奖金类型。', '獎金類型。'), { enumValues: ['welcome', 'reload', 'cashback', 'freespin', 'promo_code', 'tournament', 'voucher', 'promcash', 'other'] }),
