...
| **需要了解 Prebid.js** | 否 | 是 |
| **支持广告格式** | Banner / Pop / Push / In-page Push | Banner / Video / Native / Multi-Format |
...
adflow.js 是一个自包含的 JavaScript SDK，将 Prebid.js S2S 竞价的所有逻辑封装在一个脚本中。客户只需引入一个 `<script>` 标签并在广告位放置 `<iframe>` 标签即可完成接入，无需手动加载 Prebid.js 或编写竞价代码。支持 **Banner**（横幅）、**Pop**（弹窗）、**Push**（推送通知）、**In-page Push**（页内推送）四种广告类型，通过广告位上的 `data-ad-type` 属性切换。

[Banner 调试器](/cn/adflow/banner-debugger) · [Pop 调试器](/cn/adflow/pop-debugger) · [Push 调试器](/cn/adflow/push-debugger) · [In-page Push 调试器](/cn/adflow/inpage-push-debugger)
...
adflow.js 支持四种广告类型，脚本引入方式完全相同，区别仅在于广告位 `<iframe>` 上的 `data-ad-type` 属性。
...
| `data-ad-type` | 可选 | `banner` | 广告类型：`banner`（默认）、`pop`（弹窗）、`push`（推送通知）、`inpage-push`（页内推送） |
| `width` | 可选 | `300` / `492` | 广告位宽度(像素)。`banner` 为创意尺寸（默认 300）；`inpage-push` 为请求的图片宽度（默认 492） |
| `height` | 可选 | `250` / `328` | 广告位高度(像素)。`banner` 为创意尺寸（默认 250）；`inpage-push` 为请求的图片高度（默认 328） |
