...
| **需要了解 Prebid.js** | 否 | 是 |
| **支持广告格式** | Banner / Pop / Push | Banner / Video / Native / Multi-Format |
...
adflow.js 是一个自包含的 JavaScript SDK，将 Prebid.js S2S 竞价的所有逻辑封装在一个脚本中。客户只需引入一个 `<script>` 标签并在广告位放置 `<iframe>` 标签即可完成接入，无需手动加载 Prebid.js 或编写竞价代码。支持 **Banner**（横幅）、**Pop**（弹窗）、**Push**（推送通知）三种广告类型，通过广告位上的 `data-ad-type` 属性切换。

[Banner 调试器](/cn/adflow/banner-debugger) · [Pop 调试器](/cn/adflow/pop-debugger) · [Push 调试器](/cn/adflow/push-debugger)
...
adflow.js 支持三种广告类型，脚本引入方式完全相同，区别仅在于广告位 `<iframe>` 上的 `data-ad-type` 属性。
...
| `data-ad-type` | 可选 | `banner` | 广告类型：`banner`（默认）、`pop`（弹窗）、`push`（推送通知） |
| `width` | 可选 | `300` | 广告位宽度(像素)，仅 `banner` 类型有效 |
| `height` | 可选 | `250` | 广告位高度(像素)，仅 `banner` 类型有效 |
