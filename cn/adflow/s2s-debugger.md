...
测试 Prebid.js S2S 配置接入方案：通过 Prebid Server 转发竞价请求。
...
<div class="s2s-dbg">
  <div class="controls">
    ...
    <button id="s2s-runBtn">运行测试</button>
  </div>
</div>
</ClientOnly>
...
根据上方配置参数自动生成的 Prebid.js S2S 接入代码：
...
<div class="s2s-dbg">
  <div class="grid">
    <div class="card">
      <h3>执行状态</h3>
      <div>
        <div class="test-row"><span class="test-name">Prebid.js 加载</span><span class="status pending" id="s2s-status-prebid">等待测试</span></div>
        <div class="test-row"><span class="test-name">S2S 配置</span><span class="status pending" id="s2s-status-s2s">等待测试</span></div>
        <div class="test-row"><span class="test-name">竞价请求</span><span class="status pending" id="s2s-status-auction">等待测试</span></div>
        <div class="test-row"><span class="test-name">广告渲染</span><span class="status pending" id="s2s-status-render">等待测试</span></div>
      </div>
    </div>
    <div class="card">
      <h3>S2S 配置</h3>
...
onMounted(async () => {
  ...
  function updateCodePreview() {
    ...
    var html = '';
    html += '<span class="hl-comment">// 1. 加载 Prebid.js</span>\n';
    ...
    html += '<span class="hl-comment">// 2. 配置 S2S 并启动竞价</span>\n';
  ...
  function updateRuntimeConfig(s2sConfig, adUnit) {
    ...
    html += '<div><span class="key">账号:</span> <span class="val">' + escapeHtml(s2sConfig.accountId) + '</span></div>';
    html += '<div><span class="key">竞价方:</span> <span class="val">revosurge</span></div>';
    html += '<div><span class="key">超时:</span> <span class="val">' + s2sConfig.timeout + 'ms</span></div>';
    if (adUnit) {
  ...
  function runTest() {
    ...
    if (container) container.innerHTML = '等待竞价...';

    var serverUrl = document.getElementById('s2s-serverUrl').value.trim().replace(/\/+$/, '');
    var accountId = document.getElementById('s2s-accountId').value.trim();
    var pid1 = document.getElementById('s2s-placementId1').value.trim();

    log('========== 开始 S2S 竞价测试 ==========', 'info');
    ...
    waitForPrebid(function() {
      setStatus('s2s-status-prebid', 'success', '已加载 v' + (window.pbjs.version || '?'));
      log('Prebid.js 已加载，版本: ' + (window.pbjs.version || 'unknown'), 'success');
      ...
      window.pbjs.que.push(function() {
        ...
        log('S2S 配置已应用: ' + JSON.stringify(s2sConfig, null, 2), 'success');
        ...
        setStatus('s2s-status-auction', 'pending', '竞价中...');
        log('广告位已添加，开始发起竞价请求...');
        ...
        window.pbjs.requestBids({
          ...
          bidsBackHandler: function() {
            log('Prebid.js 竞价完成！');
            setStatus('s2s-status-auction', 'success', '竞价完成');
            ...
            if (winners && winners.length > 0) {
              var win = winners[0];
              setStatus('s2s-status-render', 'success', '竞价成功 $' + win.cpm.toFixed(4));
              log('胜出出价: ' + win.bidderCode + ' - CPM: $' + win.cpm.toFixed(4), 'success');
              ...
              if (c) {
                if (win.ad) {
                  ...
                  c.innerHTML = '<div style="text-align:center;padding:20px;">'
                    + '<div style="color:var(--vp-c-green);font-size:18px;margin-bottom:8px;font-weight:600;">竞价成功</div>'
                    + '<div style="color:var(--vp-c-text-2);font-size:13px;">'
                    + '竞价方: ' + escapeHtml(win.bidderCode) + '<br/>'
              ...
              setStatus('s2s-status-render', 'warn', '无出价');
              if (w) w.className = 'ad-slot-wrapper no-bid';
              if (c) {
                c.innerHTML = '<div style="text-align:center;padding:20px;">'
                  + '<div style="color:var(--vp-c-warning-1);font-size:16px;margin-bottom:8px;">已连接 - 无出价</div>'
  ...
  function waitForPrebid(callback) {
    ...
    setStatus('s2s-status-prebid', 'pending', '加载中...');
    log('等待 Prebid.js 加载...');
    var attempts = 0;
    var timer = setInterval(function() {
      ...
      if (typeof window.pbjs !== 'undefined') {
        ...
        clearInterval(timer);
        setStatus('s2s-status-prebid', 'error', '加载超时');
        log('Prebid.js 加载超时 (15s)', 'error');
