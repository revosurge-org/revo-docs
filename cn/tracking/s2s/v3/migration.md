...
v2 和 v3 是**独立的端点**，不存在自动转换：调用方直接发送 v3 契约。v2 将于 **2026 年 10 月 18 日**弃用。
...
6. 使用 [dry run](/cn/tracking/s2s/v3/server-events-api#dry-run)（`?dryrun=1`）进行校验——修复任何 `violations[]`。
